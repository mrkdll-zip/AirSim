...
#include "airlib/common/common_utils/ScheduledExecutor.hpp"
...
class World : public UpdatableContainer<UpdatableObject*> {
  ...
  {
    if (physics_engine_ && member->getPhysicsBody() != nullptr)
      physics_engine_->erase_remove(
          static_cast<PhysicsBody*>(member->getPhysicsBody()));
  ...
  void startAsyncUpdator(uint64_t period)
  {
    ...
    executor_.initialize(
        std::bind(&World::worldUpdatorAsync, this, std::placeholders::_1), period);
    executor_.start();
  }
  ...
  bool worldUpdatorAsync(uint64_t dt_nanos)
  {
    ...
    try {
      ...
      // Utils::DebugBreak();
      Utils::log(
          Utils::stringf("Exception occurred while updating world: %s", ex.what()),
