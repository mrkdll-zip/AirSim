...
class Environment : public UpdatableObject {
  ...
  virtual void failResetUpdateOrdering(std::string err) override
  {
    ...
    // those functions directly reset an environment, and also call other reset()s that
    // reset the same environment.
  }
  ...
  static void updateState(State& state, const HomeGeoPoint& home_geo_point)
  {
    ...
    state.air_density =
        EarthUtils::getAirDensity(state.air_pressure, state.temperature);
