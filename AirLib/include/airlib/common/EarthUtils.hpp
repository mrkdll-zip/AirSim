...
class EarthUtils {
  ...
  static constexpr int DECLINATION_TABLE[13][37] = {
      {46,  45,  44,  42,  41,  40,  38,  36,  33,  28,  23,  16,  10,
       4,   -1,  -5,  -9,  -14, -19, -26, -33, -40, -48, -55, -61, -66,
       -71, -74, -75, -72, -61, -25, 22,  40,  45,  47,  46},
      {30,  30,  30,  30,  29,  29,  29,  29,  27,  24,  18,  11,  3,
       -3,  -9,  -12, -15, -17, -21, -26, -32, -39, -45, -51, -55, -57,
       -56, -53, -44, -31, -14, 0,   13,  21,  26,  29,  30},
      {21,  22,  22,  22,  22,  22,  22,  22,  21,  18,  13,  5,   -3,
       -11, -17, -20, -21, -22, -23, -25, -29, -35, -40, -44, -45, -44,
       -40, -32, -22, -12, -3,  3,   9,   14,  18,  20,  21},
      {16,  17,  17,  17,  17,  17,  16,  16,  16,  13,  8,   0,   -9,
       -16, -21, -24, -25, -25, -23, -20, -21, -24, -28, -31, -31, -29,
       -24, -17, -9,  -3,  0,   4,   7,   10,  13,  15,  16},
      {12,  13,  13,  13,  13,  13,  12,  12, 11,  9,   3,   -4,  -12,
       -19, -23, -24, -24, -22, -17, -12, -9, -10, -13, -17, -18, -16,
       -13, -8,  -3,  0,   1,   3,   6,   8,  10,  12,  12},
  ...
  static real_T getMagDeclination(real_T latitude, real_T longitude)
  {
    ...
    if (latitude < -90.0f || latitude > 90.0f || longitude < -180.0f ||
        longitude > 180.0f) {
      throw std::out_of_range(Utils::stringf(
          "invalid latitude (%f) or longitude (%f) value", latitude, longitude));
    }
    ...
    /* limit to table bounds - required for maxima even when table spans full globe
     * range */
    ...
    if (latitude >= MAG_SAMPLING_MAX_LAT) {
      min_lat = static_cast<int>(latitude / MAG_SAMPLING_RES) * MAG_SAMPLING_RES -
                MAG_SAMPLING_RES;
    }
    ...
    if (longitude >= MAG_SAMPLING_MAX_LON) {
      min_lon = static_cast<int>(longitude / MAG_SAMPLING_RES) * MAG_SAMPLING_RES -
                MAG_SAMPLING_RES;
    }
    ...
    real_T declination_ne =
        get_mag_lookup_table_val(min_lat_index + 1, min_lon_index + 1);
    real_T declination_nw = get_mag_lookup_table_val(min_lat_index + 1, min_lon_index);

    /* perform bilinear interpolation on the four grid corners */

    real_T declination_min =
        ((longitude - min_lon) / MAG_SAMPLING_RES) * (declination_se - declination_sw) +
        declination_sw;
    real_T declination_max =
        ((longitude - min_lon) / MAG_SAMPLING_RES) * (declination_ne - declination_nw) +
        declination_nw;

    return ((latitude - min_lat) / MAG_SAMPLING_RES) *
               (declination_max - declination_min) +
           declination_min;
  }

  // geopot_height = Earth_radius * altitude / (Earth_radius + altitude) /// all in
  // kilometers temperature is in Kelvin = 273.15 + celcius
  static real_T getStandardTemperature(real_T geopot_height)
  {
    ...
    // throw std::out_of_range("geopot_height must be less than 85km. Space domain is
    // not supported yet!");
  }
  ...
  static real_T getStandardPressure(real_T geopot_height,
                                    real_T std_temperature)  // return Pa
  {
    // Below 51km: Practical Meteorology by Roland Stull, pg 12
    // Above 51km: http://www.braeunig.us/space/atmmodel.htm
    // Validation data:
    ...
    if (geopot_height <= 11)
      ...
      return 1E-3f;
    // throw std::out_of_range("altitude must be less than 86km. Space domain is not
    // supported yet!");
  }
  ...
  static real_T getGravity(real_T altitude)
  {
    ...
    if (altitude < 100000 &&
        altitude > -100000)  // use first exproximation using binomial expansion
  ...
  static Vector3r getMagField(const GeoPoint& geo_point, double& declination,
                              double& inclination)  // return Tesla
  {
    /*
...
This obviously is inaccurate because of huge amount of irregularities, magnetic pole
that is constantly moving, shape of Earth, higher order terms, dipole that is not
perfectly aligned etc. For simulation we are not looking for actual values of magnetic
field but rather if field changes correctly as vehicle moves in any direction and if
field component signs are correct. For this purpose, simple diapole model is good
enough. Keep in mind that actual field values may differ by as much as 10X in either
direction although for many tests differences seems to be within 3X or sometime even to
first decimal digit. Again what matters is how field changes wrt to movement as opposed
to actual field values. To get better field strength one should use latest World
Magnetic Model like WMM2015 from NOAA. However these recent model is fairly complex and
very expensive to calculate. Other possibilities:
    - WMM2010 mocel, expensive to compute:
http://williams.best.vwh.net/magvar/magfield.c
    - Android's mag field calculation (still uses WMM2010 and fails at North Pole):
https://goo.gl/1CZB9x

Performance:
    This function takes about 1 microsecond on Lenovo P50 laptop (Intel Xeon E3-1505M v5
CPU) Basic trignometry functions runs at 30ns.

Accuracy:
    Two points separated by sqrt(2 km)
    Dipole Model:   2.50394e-05     3.40771e-06     3.6567e-05  (dec: 7.7500,
inc: 55.3530)
    WMM2015 Model:  1.8350e-05		5.201e-06		5.0158e-05  (dec: 15.8248,
inc: 69.1805) geo:            47.637  -122.147    622

    Dipole Model:   2.5047e-05      3.41024e-06     3.65953e-05 (dec: 7.7536,
inc: 55.36532)
    WMM2015 Model:  1.8353e-05		5.203e-06		5.0191e-05  (dec: 15.8278,
inc: 69.1897) geo:            47.646  -122.134    -378
*/

    // ref: The Earth's Magnetism: An Introduction for Geologists, Roberto Lanza,
    // Antonio Meloni Sec 1.2.5, pg 27-30 https://goo.gl/bRm7wt some theory at
    ...
    // calculation of magnetic longitude is not needed but just in case if someone wants
    // it double mag_lon = asin(
  ...
  static GeoPoint nedToGeodetic(const Vector3r& v, const HomeGeoPoint& home_geo_point)
  {
    ...
    if (!Utils::isApproximatelyZero(c)) {  // avoids large changes?
      lat_rad = asin(cos_c * home_geo_point.sin_lat +
                     (x_rad * sin_c * home_geo_point.cos_lat) / c);
      lon_rad = (home_geo_point.lon_rad +
                 atan2(y_rad * sin_c, c * home_geo_point.cos_lat * cos_c -
                                          x_rad * home_geo_point.sin_lat * sin_c));

      return GeoPoint(Utils::radiansToDegrees(lat_rad),
                      Utils::radiansToDegrees(lon_rad),
                      home_geo_point.home_geo_point.altitude - v.z());
    }
    else
      return GeoPoint(home_geo_point.home_geo_point.latitude,
                      home_geo_point.home_geo_point.longitude,
                      home_geo_point.home_geo_point.altitude - v.z());
  }
  ...
  static Vector3r GeodeticToEcef(const GeoPoint& geo)
  {
    ...
    real_T x = static_cast<real_T>((EARTH_RADIUS / xi + geo.altitude) * cos(lat_rad) *
                                   cos(lon_rad));
    real_T y = static_cast<real_T>((EARTH_RADIUS / xi + geo.altitude) * cos(lat_rad) *
                                   sin(lon_rad));
    real_T z = static_cast<real_T>(
        (EARTH_RADIUS / xi * (1 - (6.69437999014 * 0.001)) + geo.altitude) *
        sin(lat_rad));
    return Vector3r(x, y, z);
  }

  static Vector3r EcefToNed(const Vector3r& ecefxyz, const Vector3r& ecefhome,
                            const GeoPoint& geohome)
  {
  ...
  // below are approximate versions and would produce errors of more than 10m for points
  // farther than 1km for more accurate versions, please use the version in
  // EarthUtils::nedToGeodetic
  ...
  static GeoPoint nedToGeodeticFast(const Vector3r& local, const GeoPoint& home)
  {
    ...
    double d_lon =
        local.y() / (EARTH_RADIUS * cos(Utils::degreesToRadians(home.latitude)));
  ...
  static constexpr double Perihelion =
      Utils::degreesToRadians(102.9372);                     // perihelion of the Earth
