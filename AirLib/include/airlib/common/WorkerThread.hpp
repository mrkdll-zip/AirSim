...
// This wraps a condition_variable so we can handle the case where we may signal before
// wait and implement the semantics that say wait should be a noop in that case.
class WorkerThreadSignal {
  ...
  bool waitFor(double timeout_sec)
  {
    ...
    while (!signaled_) {
      std::unique_lock<std::mutex> lock(mutex_);
      cv_.wait_for(
          lock, std::chrono::milliseconds(static_cast<long long>(timeout_sec * 1000)));
    }
    signaled_ = false;
    return true;
  }
    ...
    while (!cancel_request_ && pending_item_ != nullptr) {
      ...
      if (pending != nullptr && !pending->isCancelled()) {
        ...
        try {
          ...
          // Utils::DebugBreak();
          Utils::log(
              Utils::stringf("WorkerThread caught unhandled exception: %s", e.what()),
