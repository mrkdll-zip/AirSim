...
#include "airlib/sensors/SensorBase.hpp"
...
struct AirSimSettings {
  ...
  struct RecordingSetting {
    ...
    RecordingSetting(bool record_on_move_val, float record_interval_val,
                     const std::string& folder_val, bool enabled_val)
  ...
  struct PawnPath {
    ...
    PawnPath(const std::string& pawn_bp_val = "",
             const std::string& slippery_mat_val =
                 "/AirSim/VehicleAdv/PhysicsMaterials/Slippery.Slippery",
             const std::string& non_slippery_mat_val =
                 "/AirSim/VehicleAdv/PhysicsMaterials/NonSlippery.NonSlippery")
        : pawn_bp(pawn_bp_val),
          slippery_mat(slippery_mat_val),
          non_slippery_mat(non_slippery_mat_val)
    {
    }
  };
  ...
  struct CaptureSetting {
    ...
    float target_gamma = Utils::nan<float>();  // 1.0f; //This would be reset to
  ...
  struct SensorSetting {
    ...
    bool enabled = true;
    Settings
        settings;  // imported json data that needs to be parsed by specific sensors.
  };
  ...
  struct VehicleSetting {
    ...
    VehicleSetting(const std::string& vehicle_name_val,
                   const std::string& vehicle_type_val)
  ...
  struct MavLinkConnectionInfo {
    ...
    // Used to connect to drone over UDP: needed only if use_serial = false and use_tcp
    // == false
    ...
    // The PX4 SITL app requires receiving drone commands over a different mavlink
    // channel called the "ground control station" (or GCS) channel.
    std::string control_ip_address = "127.0.0.1";
    int control_port_local         = 14540;
    int control_port_remote        = 14580;

    // The log viewer can be on a different machine, so you can configure it's ip
    ...
    // The QGroundControl app can be on a different machine, and AirSim can act as a
    // proxy to forward the mavlink stream over to that machine if you configure it's ip
    // address and port here.
    int qgc_ip_port            = 0;
    ...
    // if you want to select a specific local network adapter so you can reach certain
    // remote machines (e.g. wifi versus ethernet) then you will want to change the
    // LocalHostIp accordingly.  This default only works when log viewer and QGC are
    // also on the same machine.  Whatever network you choose it has to be the same one
    // for external
  ...
  void load(std::function<std::string(void)> simmode_getter)
  {
    ...
    // this should be done last because it depends on vehicles (and/or their type) we
    // have
  ...
  static void createDefaultSettingsFile()
  {
    ...
    // write some settings_json in new file otherwise the string "null" is written if
    // all settings_json are empty
    settings_json.setString(
        "SeeDocsAt",
    ...
    // TODO: there is a crash in Linux due to settings_json.saveJSonString(). Remove
    // this workaround after we only support Unreal 4.17
  ...
  void addVehicleSetting(const std::string& vehicle_name,
                         const std::string& vehicle_type, const Pose& pose,
                         const std::string& pawn_path = "")
  {
    // No Mavlink-type vehicles currently
    auto vehicle_setting =
        std::unique_ptr<VehicleSetting>(new VehicleSetting(vehicle_name, vehicle_type));
        ...
        if (settings_version_actual == 1) {
          ...
          if (detected_keys.length()) {
            std::string error_message =
                "You are using newer version of AirSim with older version of "
                "settings.json. "
                "You can either delete your settings.json and restart AirSim or use "
                "the upgrade "
  ...
  bool hasDefaultSettings(const Settings& settings_json, float& version)
  {
    ...
    // we had spelling mistake so we are currently supporting SettingsVersion or
    // SettingdVersion :(
    version = settings_json.getFloat("SettingsVersion",
                                     settings_json.getFloat("SettingdVersion", 0));
  ...
  void loadViewModeSettings(const Settings& settings_json)
  {
    ...
    if (view_mode_string == "Fpv")
      ...
      initial_view_mode =
          1;  // ECameraDirectorMode::CAMERA_DIRECTOR_MODE_GROUND_OBSERVER;
      ...
      initial_view_mode =
          4;  // ECameraDirectorMode::CAMERA_DIRECTOR_MODE_SPRINGARM_CHASE;
  ...
  static void loadRCSetting(const std::string& simmode_name,
                            const Settings& settings_json, RCSettings& rc_setting)
  {
    Settings rc_json;
    if (settings_json.getChild("RC", rc_json)) {
      rc_setting.remote_control_id = rc_json.getInt(
          "RemoteControlID", simmode_name == kSimModeTypeMultirotor ? 0 : -1);
      rc_setting.allow_api_when_disconnected = rc_json.getBool(
          "AllowAPIWhenDisconnected", rc_setting.allow_api_when_disconnected);
    }
  }
  ...
  static std::string getCameraName(const Settings& settings_json)
  {
    return settings_json.getString("CameraName",
                                   // TODO: below exist only due to legacy reason and
                                   // can be replaced by "" in future
  ...
  void loadRecordingSetting(const Settings& settings_json)
  {
    ...
    if (settings_json.getChild("Recording", recording_json)) {
      recording_setting.record_on_move =
          recording_json.getBool("RecordOnMove", recording_setting.record_on_move);
      recording_setting.record_interval =
          recording_json.getFloat("RecordInterval", recording_setting.record_interval);
      recording_setting.folder =
          recording_json.getString("Folder", recording_setting.folder);
      recording_setting.enabled =
          recording_json.getBool("Enabled", recording_setting.enabled);
      ...
      if (recording_json.getChild("Cameras", req_cameras_settings)) {
        ...
        for (size_t child_index = 0; child_index < req_cameras_settings.size();
             ++child_index) {
          ...
          if (req_cameras_settings.getChild(child_index, req_camera_settings)) {
            ...
            ImageType image_type =
                Utils::toEnum<ImageType>(req_camera_settings.getInt("ImageType", 0));
            ...
            std::string vehicle_name =
                req_camera_settings.getString("VehicleName", default_vehicle_name);

            recording_setting.requests[vehicle_name].push_back(
                ImageCaptureBase::ImageRequest(camera_name, image_type, pixels_as_float,
                                               compress));
          }
        }
      }
  ...
  static void loadCaptureSettings(const Settings& settings_json,
                                  std::map<int, CaptureSetting>& capture_settings)
  {
    initializeCaptureSettings(capture_settings);
  ...
  static std::unique_ptr<VehicleSetting> createMavLinkVehicleSetting(
      const Settings& settings_json)
  {
    ...
    MavLinkVehicleSetting* vehicle_setting =
        static_cast<MavLinkVehicleSetting*>(vehicle_setting_p.get());
    ...
    connection_info.sim_sysid              = static_cast<uint8_t>(
        settings_json.getInt("SimSysID", connection_info.sim_sysid));
    connection_info.sim_compid =
        settings_json.getInt("SimCompID", connection_info.sim_compid);

    connection_info.vehicle_sysid = static_cast<uint8_t>(
        settings_json.getInt("VehicleSysID", connection_info.vehicle_sysid));
    connection_info.vehicle_compid =
        settings_json.getInt("VehicleCompID", connection_info.vehicle_compid);

    connection_info.offboard_sysid = static_cast<uint8_t>(
        settings_json.getInt("OffboardSysID", connection_info.offboard_sysid));
    connection_info.offboard_compid =
        settings_json.getInt("OffboardCompID", connection_info.offboard_compid);

    connection_info.logviewer_ip_address = settings_json.getString(
        "LogViewerHostIp", connection_info.logviewer_ip_address);
    connection_info.logviewer_ip_port =
        settings_json.getInt("LogViewerPort", connection_info.logviewer_ip_port);
    connection_info.logviewer_ip_sport =
        settings_json.getInt("LogViewerSendPort", connection_info.logviewer_ip_sport);

    connection_info.qgc_ip_address =
        settings_json.getString("QgcHostIp", connection_info.qgc_ip_address);
    connection_info.qgc_ip_port =
        settings_json.getInt("QgcPort", connection_info.qgc_ip_port);

    connection_info.control_ip_address =
        settings_json.getString("ControlIp", connection_info.control_ip_address);
    connection_info.control_port_local = settings_json.getInt(
        "ControlPort", connection_info.control_port_local);  // legacy
    ...
    std::string sitlip =
        settings_json.getString("SitlIp", connection_info.control_ip_address);
    ...
    connection_info.use_serial =
        settings_json.getBool("UseSerial", connection_info.use_serial);
    connection_info.udp_address =
        settings_json.getString("UdpIp", connection_info.udp_address);
    connection_info.udp_port =
        settings_json.getInt("UdpPort", connection_info.udp_port);
    connection_info.use_tcp = settings_json.getBool("UseTcp", connection_info.use_tcp);
    connection_info.lock_step =
        settings_json.getBool("LockStep", connection_info.lock_step);
    connection_info.tcp_port =
        settings_json.getInt("TcpPort", connection_info.tcp_port);
    connection_info.serial_port =
        settings_json.getString("SerialPort", connection_info.serial_port);
    connection_info.baud_rate =
        settings_json.getInt("SerialBaudRate", connection_info.baud_rate);
  ...
  static std::unique_ptr<VehicleSetting> createVehicleSetting(
      const std::string& simmode_name, const Settings& settings_json,
      ...
      std::map<std::string, std::shared_ptr<SensorSetting>>& sensor_defaults)
  {
    ...
    vehicle_setting->allow_api_always =
        settings_json.getBool("AllowAPIAlways", vehicle_setting->allow_api_always);
    ...
    vehicle_setting->enable_collision_passthrough = settings_json.getBool(
        "EnableCollisionPassthrogh", vehicle_setting->enable_collision_passthrough);
    ...
    vehicle_setting->is_fpv_vehicle =
        settings_json.getBool("IsFpvVehicle", vehicle_setting->is_fpv_vehicle);
  ...
  static void createDefaultVehicle(
      ...
      const std::map<std::string, std::shared_ptr<SensorSetting>>& sensor_defaults)
  {
    ...
    if (simmode_name == kSimModeTypeMultirotor) {
      ...
      auto simple_flight_setting = std::unique_ptr<VehicleSetting>(
          new VehicleSetting("SimpleFlight", kVehicleTypeSimpleFlight));
      // TODO: we should be selecting remote if available else keyboard
      // currently keyboard is not supported so use rc as default
      ...
      auto physx_car_setting = std::unique_ptr<VehicleSetting>(
          new VehicleSetting("PhysXCar", kVehicleTypePhysXCar));
      ...
      auto cv_setting = std::unique_ptr<VehicleSetting>(
          new VehicleSetting("ComputerVision", kVehicleTypeComputerVision));
      cv_setting->sensors                = sensor_defaults;
      vehicles[cv_setting->vehicle_name] = std::move(cv_setting);
    }
    else {
      throw std::invalid_argument(
          Utils::stringf("Unknown SimMode: %s, failed to set default vehicle settings",
                         simmode_name.c_str())
              .c_str());
    }
  }
  ...
  static void initializePawnPaths(std::map<std::string, PawnPath>& pawn_paths)
  {
    ...
    pawn_paths.emplace(
        ...
        PawnPath(
            "Class'/AirSim/Blueprints/BP_ComputerVisionPawn.BP_ComputerVisionPawn_C'"));
  }
  ...
  static void loadSegmentationSetting(const Settings& settings_json,
                                      SegmentationSetting& segmentation_setting)
  {
    Settings json_parent;
    if (settings_json.getChild("SegmentationSettings", json_parent)) {
      ...
      if (init_method == "" || init_method == "commonobjectsrandomids")
        ...
        // TODO: below exception doesn't actually get raised right now because of issue
        // in Unreal Engine?
        throw std::invalid_argument(
            std::string(
                "SegmentationSetting init_method has invalid value in settings_json ") +
            init_method);

      segmentation_setting.override_existing =
          json_parent.getBool("OverrideExisting", true);
      ...
      if (mesh_naming_method == "" || mesh_naming_method == "ownername")
        ...
        throw std::invalid_argument(std::string("SegmentationSetting MeshNamingMethod "
                                                "has invalid value in settings_json ") +
                                    mesh_naming_method);
    }
  }
  ...
  static void initializeNoiseSettings(std::map<int, NoiseSetting>& noise_settings)
  {
    ...
    for (int i = -1; i < image_count; ++i) noise_settings[i] = NoiseSetting();
  }
  ...
  static void loadNoiseSetting(const msr::airlib::Settings& settings_json,
                               NoiseSetting& noise_setting)
  {
    ...
    noise_setting.HorzWaveStrength =
        settings_json.getFloat("HorzWaveStrength", noise_setting.HorzWaveStrength);
    ...
    noise_setting.HorzWaveContrib =
        settings_json.getFloat("HorzWaveContrib", noise_setting.HorzWaveContrib);
    noise_setting.HorzWaveVertSize =
        settings_json.getFloat("HorzWaveVertSize", noise_setting.HorzWaveVertSize);
  ...
  {
    GimbalSetting gimbal;
    // capture_setting.gimbal.is_world_frame = settings_json.getBool("IsWorldFrame",
    // false);
  ...
  static void loadCameraSettings(const Settings& settings_json,
                                 std::map<std::string, CameraSetting>& cameras)
  {
    cameras.clear();
  ...
  static void createCaptureSettings(const msr::airlib::Settings& settings_json,
                                    CaptureSetting& capture_setting)
  {
    ...
    capture_setting.auto_exposure_max_brightness = settings_json.getFloat(
        "AutoExposureMaxBrightness", capture_setting.auto_exposure_max_brightness);
    capture_setting.auto_exposure_min_brightness = settings_json.getFloat(
        "AutoExposureMinBrightness", capture_setting.auto_exposure_min_brightness);
    ...
    capture_setting.target_gamma =
        settings_json.getFloat("TargetGamma", capture_setting.image_type == 0
                                                  ? CaptureSetting::kSceneTargetGamma
                                                  : Utils::nan<float>());
    ...
    if (projection_mode == "" || projection_mode == "perspective")
      ...
      throw std::invalid_argument(
          std::string(
              "CaptureSettings projection_mode has invalid value in settings_json ") +
          projection_mode);
    ...
    if (settings_json.getChild("SubWindows", json_parent)) {
      for (size_t child_index = 0; child_index < json_parent.size(); ++child_index) {
        ...
        if (json_parent.getChild(child_index, json_settings_child)) {
          ...
          subwindow_setting.image_type =
              Utils::toEnum<ImageType>(json_settings_child.getInt("ImageType", 0));
          ...
          subwindow_setting.vehicle_name =
              json_settings_child.getString("VehicleName", "");
      ...
      std::vector<SubwindowSetting>& subwindow_settings)
  {
    ...
    subwindow_settings.push_back(
        SubwindowSetting(0, ImageType::DepthVis, false, "", "", false));  // depth
    subwindow_settings.push_back(
        SubwindowSetting(1, ImageType::Segmentation, false, "", "", false));  // seg
    subwindow_settings.push_back(
        SubwindowSetting(2, ImageType::Scene, false, "", "", false));  // vis
  }

  void loadOtherSettings(const Settings& settings_json)
  {
    // by default we spawn server at local endpoint. Do not use 127.0.0.1 as default
    // below because for docker container default is 0.0.0.0 and people get really
    // confused why things don't work
    ...
    {  // load origin geopoint
      ...
      if (settings_json.getChild("OriginGeopoint", origin_geopoint_json)) {
        ...
        origin.longitude =
            origin_geopoint_json.getDouble("Longitude", origin.longitude);
    ...
    {  // time of day settings_json
      ...
      if (settings_json.getChild("TimeOfDay", tod_settings_json)) {
        ...
        tod_setting.start_datetime =
            tod_settings_json.getString("StartDateTime", tod_setting.start_datetime);
        tod_setting.celestial_clock_speed = tod_settings_json.getFloat(
            "CelestialClockSpeed", tod_setting.celestial_clock_speed);
        tod_setting.is_start_datetime_dst = tod_settings_json.getBool(
            "StartDateTimeDst", tod_setting.is_start_datetime_dst);
        tod_setting.update_interval_secs = tod_settings_json.getFloat(
            "UpdateIntervalSecs", tod_setting.update_interval_secs);
        tod_setting.move_sun =
            tod_settings_json.getBool("MoveSun", tod_setting.move_sun);
      }
    }
                                        ...
                                        const std::string& simmode_name)
  {
    ...
    if (settings_json.getChild("CameraDirector", child_json)) {
      camera_director.position =
          createVectorSetting(child_json, camera_director.position);
      camera_director.rotation =
          createRotationSetting(child_json, camera_director.rotation);
      camera_director.follow_distance =
          child_json.getFloat("FollowDistance", camera_director.follow_distance);
    }
  ...
  static std::shared_ptr<SensorSetting> createSensorSetting(
      SensorBase::SensorType sensor_type, const std::string& sensor_name, bool enabled)
  {
  ...
  static void initializeSensorSetting(SensorSetting* sensor_setting,
                                      const Settings& settings_json)
  {
    sensor_setting->enabled = settings_json.getBool("Enabled", sensor_setting->enabled);

    // pass the json Settings property bag through to the specific sensor params object
    // where it is extracted there.  This way default values can be kept in one place.
    // For example, see the BarometerSimpleParams::initializeFromSettings method.
    sensor_setting->settings = settings_json;
  }

  // creates and intializes sensor settings from json
  static void loadSensorSettings(
      const Settings& settings_json, const std::string& collectionName,
      std::map<std::string, std::shared_ptr<SensorSetting>>& sensors,
      std::map<std::string, std::shared_ptr<SensorSetting>>& sensor_defaults)

  {
    ...
    if (settings_json.getChild(collectionName, sensors_child)) {
      ...
      for (const auto& key : keys) {
        ...
        auto sensor_type =
            Utils::toEnum<SensorBase::SensorType>(child.getInt("SensorType", 0));
      ...
      std::map<std::string, std::shared_ptr<SensorSetting>>& sensors)
  {
    if (simmode_name == kSimModeTypeMultirotor) {
      ...
      sensors["magnetometer"] = createSensorSetting(
          SensorBase::SensorType::Magnetometer, "magnetometer", true);
      sensors["gps"] = createSensorSetting(SensorBase::SensorType::Gps, "gps", true);
      sensors["barometer"] =
          createSensorSetting(SensorBase::SensorType::Barometer, "barometer", true);
    }
    else if (simmode_name == kSimModeTypeCar) {
      sensors["gps"] = createSensorSetting(SensorBase::SensorType::Gps, "gps", true);
    }
  ...
  static void loadExternalCameraSettings(
      const Settings& settings_json,
      std::map<std::string, CameraSetting>& external_cameras)
  {
