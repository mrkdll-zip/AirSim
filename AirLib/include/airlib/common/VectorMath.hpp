...
#include "airlib/common/common_utils/Utils.hpp"
#include "common_utils/RandomGenerator.hpp"

STRICT_MODE_OFF
// if not using unaligned types then disable vectorization to avoid alignment issues all
// over the places #define EIGEN_DONT_VECTORIZE
#include "Eigen/Dense"
STRICT_MODE_ON
...
class VectorMathT {
  ...
  // TODO: below we are using double instead of RealT because of VC++2017 bug in random
  // implementation
  typedef common_utils::RandomGenerator<RealT, std::normal_distribution<double>, 1>
      RandomGeneratorGausianXT;
  typedef common_utils::RandomGenerator<RealT, std::normal_distribution<double>, 2>
      RandomGeneratorGausianYT;
  typedef common_utils::RandomGenerator<RealT, std::normal_distribution<double>, 3>
      RandomGeneratorGausianZT;
  typedef common_utils::RandomGenerator<RealT, std::uniform_real_distribution<RealT>, 1>
      RandomGeneratorXT;
  typedef common_utils::RandomGenerator<RealT, std::uniform_real_distribution<RealT>, 2>
      RandomGeneratorYT;
  typedef common_utils::RandomGenerator<RealT, std::uniform_real_distribution<RealT>, 3>
      RandomGeneratorZT;
  ...
  struct Pose {
    ...
    friend bool operator==(const Pose& lhs, const Pose& rhs)
    {
      return lhs.position == rhs.position &&
             lhs.orientation.coeffs() == rhs.orientation.coeffs();
    }
    friend bool operator!=(const Pose& lhs, const Pose& rhs)
    {
      return !(lhs == rhs);
    ...
    static Pose nanPose()
    {
      static const Pose nan_pose(VectorMathT::nanVector(),
                                 VectorMathT::nanQuaternion());
      return nan_pose;
    }
    static Pose zero()
    {
...
public:
    ...
    RandomVectorT(const Vector3T& min_val, const Vector3T& max_val)
        : rx_(min_val.x(), max_val.x()),
          ry_(min_val.y(), max_val.y()),
          rz_(min_val.z(), max_val.z())
    {
    }
...
private:
  ...
  static Vector3T rotateVector(const Vector3T& v, const QuaternionT& q,
                               bool assume_unit_quat)
  {
    unused(assume_unit_quat);  // stop warning: unused parameter.
                               // More performant method is at
                               // http://gamedev.stackexchange.com/a/50545/20758
                               // QuaternionT vq(0, v.x(), v.y(), v.z());
                               // QuaternionT qi = assume_unit_quat ? q.conjugate() :
                               // q.inverse(); return (q * vq * qi).vec();

    return q._transformVector(v);
  }

  static Vector3T rotateVectorReverse(const Vector3T& v, const QuaternionT& q,
                                      bool assume_unit_quat)
  {
    // QuaternionT vq(0, v.x(), v.y(), v.z());
  ...
  static QuaternionT rotateQuaternion(const QuaternionT& q, const QuaternionT& ref,
                                      bool assume_unit_quat)
  {
    if (assume_unit_quat) {
  ...
  static QuaternionT rotateQuaternionReverse(const QuaternionT& q,
                                             const QuaternionT& ref,
                                             bool assume_unit_quat)
  {
    if (assume_unit_quat) {
      QuaternionT ref_n   = ref;
  ...
  static Vector3T transformToBodyFrame(const Vector3T& v_world,
                                       const QuaternionT& q_world,
  ...
  static Pose transformToBodyFrame(const Pose& pose_world, const Pose& body_world,
                                   bool assume_unit_quat = true)
  {
    // translate
    ...
    // rotate vector
    Vector3T v_body =
        transformToBodyFrame(translated, body_world.orientation, assume_unit_quat);
    // rotate orientation
    QuaternionT q_body = rotateQuaternionReverse(
        pose_world.orientation, body_world.orientation, assume_unit_quat);

    return Pose(v_body, q_body);
  }

  static Vector3T transformToWorldFrame(const Vector3T& v_body,
                                        const QuaternionT& q_world,
                                        bool assume_unit_quat = true)
  {
                                        ...
                                        bool assume_unit_quat = true)
  {
    // rotate
    Vector3T v_world =
        transformToWorldFrame(v_body, body_world.orientation, assume_unit_quat);
    // translate
    return v_world + body_world.position;
  }

  // transform pose specified in body frame to world frame. The body frame in world
  // coordinate is at body_world
  static Pose transformToWorldFrame(const Pose& pose_body, const Pose& body_world,
                                    bool assume_unit_quat = true)
  {
    // rotate position
    Vector3T v_world = transformToWorldFrame(pose_body.position, body_world.orientation,
                                             assume_unit_quat);
    // rotate orientation
    QuaternionT q_world = rotateQuaternion(pose_body.orientation,
                                           body_world.orientation, assume_unit_quat);
    // translate
  ...
  {
    return Vector3T(Utils::getRandomFromGaussian(stddev, mean),
  ...
  static void toEulerianAngle(const QuaternionT& q, RealT& pitch, RealT& roll,
                              RealT& yaw)
  {
  ...
  static RealT angleBetween(const Vector3T& v1, const Vector3T& v2,
  ...
  static Vector3T toAngularVelocity(const QuaternionT& start, const QuaternionT& end,
                                    RealT dt)
  {
    if (dt == 0)
  ...
  {
    static const Vector3T val(std::numeric_limits<RealT>::quiet_NaN(),
  ...
  {
    return QuaternionT(std::numeric_limits<RealT>::quiet_NaN(),
                       std::numeric_limits<RealT>::quiet_NaN(),
                       std::numeric_limits<RealT>::quiet_NaN(),
                       std::numeric_limits<RealT>::quiet_NaN());
  }
  ...
  static QuaternionT addAngularVelocity(const QuaternionT& orientation,
                                        const Vector3T& angular_vel, RealT dt)
  {
    QuaternionT dq_unit = QuaternionT(0, angular_vel.x() * 0.5f, angular_vel.y() * 0.5f,
                                      angular_vel.z() * 0.5f) *
                          orientation;
  ...
  {
    QuaternionT tmp(0, lhs.position.x() - rhs.position.x(),
  ...
  static QuaternionT coordOrientationSubtract(const QuaternionT& lhs,
                                              const QuaternionT& rhs)
  {
  ...
  {
    return Pose(coordPositionSubtract(lhs, rhs),
                coordOrientationSubtract(lhs.orientation, rhs.orientation));
  }
  static Pose add(const Pose& lhs, const Pose& rhs)
  {
    return Pose(coordPositionAdd(lhs, rhs),
                coordOrientationAdd(lhs.orientation, rhs.orientation));
  }
  ...
  {
    if (!add_eularian)
      return Utils::stringf("[%f, %f, %f, %f]", quaternion.w(), quaternion.x(),
                            quaternion.y(), quaternion.z());
    else {
      RealT pitch, roll, yaw;
      toEulerianAngle(quaternion, pitch, roll, yaw);
      return Utils::stringf("[%f, %f, %f, %f]-[%f, %f, %f]", quaternion.w(),
                            quaternion.x(), quaternion.y(), quaternion.z(), pitch, roll,
                            yaw);
    }
  }
   ...
   * angles. RPY rotates about the fixed axes in the order x-y-z, which is the same as
   * euler angles in the order z-y'-x''.
   */
...
//below is manual way to do this
        RealT n_alpha = 1 - alpha;
        RealT theta = acos(from.x()*to.x() + from.y()*to.y() + from.z()*to.z() +
from.w()*to.w());
...
*/
  ...
  static Vector3T slerp(const Vector3T& from, const Vector3T& to, RealT alpha,
  ...
  static void getPlaneOrthoVectors(const Vector3T& from, const Vector3T& to,
                                   bool assume_normalized, Vector3T& from_ortho,
                                   Vector3T& to_ortho, RealT& dot)
  {
    unused(from);
  ...
  // assuming you are looking at front() vector, what rotation you need to look at
  // destPoint?
...
*/
  ...
  // what rotation we need to rotate "" vector to "to" vector (rotation is around
...
typedef VectorMathT<Eigen::Vector3d, Eigen::Quaternion<double, Eigen::DontAlign>,
                    double>
    VectorMathd;
typedef VectorMathT<Eigen::Vector3f, Eigen::Quaternion<float, Eigen::DontAlign>, float>
    VectorMathf;
}  // namespace airlib
}  // namespace msr
#endif
