...
#include "airlib/common/CancelToken.hpp"
...
class Waiter {
  public:
  Waiter(TTimeDelta sleep_duration_seconds, TTimeDelta timeout_sec,
         CancelToken& cancelable_action)
      : sleep_duration_(sleep_duration_seconds),
        timeout_sec_(timeout_sec),
        cancelable_action_(cancelable_action),
        is_complete_(false)
  {
  ...
  bool sleep()
  {
    // Sleeps for the time needed to get current running time up to the requested
    // sleep_duration_. So this can be used to "throttle" any loop to check something
    // every sleep_duration_ seconds.

    if (isComplete())
      throw std::domain_error(
          "Process was already complete. This instance of Waiter shouldn't be reused!");
    if (isTimeout())
      return false;
