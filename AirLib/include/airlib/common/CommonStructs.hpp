...
struct GeoPoint {
  ...
  {
    return std::to_string(latitude) + string(", ") + std::to_string(longitude) +
           string(", ") + std::to_string(altitude);
  }
};
...
struct ProjectionMatrix {
  ...
  {
    for (auto i = 0; i < 4; ++i)
      for (auto j = 0; j < 4; ++j) matrix[i][j] = val;
  }
};
...
struct DetectionInfo {
  ...
  DetectionInfo(const std::string& name_val, const GeoPoint& geo_point_val,
                const Box2D& box2D_val, const Box3D& box3D_val,
                const Pose& relative_pose_val)
      : name(name_val),
        geo_point(geo_point_val),
        box2D(box2D_val),
        box3D(box3D_val),
...
struct RCData {
  ...
  string toString()
  {
    return Utils::stringf("RCData[pitch=%f, roll=%f, throttle=%f, yaw=%f]", pitch, roll,
                          throttle, yaw);
  }
};
...
struct LidarData {
  TTimePoint time_stamp = 0;
  // data
  // - array of floats that represent [x,y,z] coordinate for each point hit within the
  // range
...
struct CameraDetails {
  ...
  std::string to_string() const
  {
    return Utils::stringf("CameraDetails: camera_name=%s, vehicle_name=%s, external=%d",
                          camera_name.c_str(), vehicle_name.c_str(), external);
  }
};
}  // namespace airlib
}  // namespace msr
