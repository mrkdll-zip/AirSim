...
// This is an abstraction for cameras associated with a vehicle.  Each camera has a
// unique id.
class ImageCaptureBase {
  public:                       // types
  enum class ImageType : int {  // this indexes to array, -1 is special to indicate main
                                // camera
    Scene = 0,
    DepthPlanar,
    DepthPerspective,
  ...
  virtual void getImages(const std::vector<ImageRequest>& requests,
                         std::vector<ImageResponse>& responses) const = 0;
