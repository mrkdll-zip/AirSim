...
class EarthCelestial {
  ...
  static CelestialLocalCoord getMoonCoordinates(uint64_t date, double lat, double lng)
  {
    ...
    double pa = std::atan2(std::sin(H), std::tan(phi) * std::cos(c.declination) -
  ...
  static CelestialPhase getMoonPhase(uint64_t date)
  {
    ...
    double sdist =
        EarthUtils::DistanceFromSun / 1000;  // distance from Earth to Sun in km
  ...
  {
    return std::atan2(std::sin(l) * std::cos(EarthUtils::Obliquity) -
  ...
  static double astroRefraction(double h)
  {
    ...
    // (Willmann-Bell, Richmond) 1998. 1.02 / tan(h + 10.26 / (h + 5.10)) h in degrees,
  ...
  {
    double C =
        Utils::degreesToRadians((1.9148 * std::sin(M) + 0.02 * std::sin(2 * M) +
  ...
  static CelestialGlobalCoord getGlobalMoonCoords(double d)
  {
    ...
    double L =
        Utils::degreesToRadians((218.316 + 13.176396 * d));  // ecliptic longitude
