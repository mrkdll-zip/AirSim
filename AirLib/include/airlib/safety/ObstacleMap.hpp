...
information. The segment between ticks 0-1 is 0, 1-2 is 1 and so on. So this scheme
allows us to get the front cone always at segment 0. If we have only 4 sensors like in
DJI Matrice Guidance system then segment 1 has information about obstacles on right, seg
2 for back and so on.

Another design criteria is that this class is thread safe for concurrent updates and
queries. We fully expect one thread to continuously update the obstacles while another
to query the map.
*/
...
class ObstacleMap {
  ...
  // what is the confidence in these values? This should typically be the standard
  // deviation
  ...
  struct ObstacleInfo {
    ...
    {
      return Utils::stringf("Obs: tick=%i, distance=%f, confidence=%f", tick, distance,
                            confidence);
    }
  };
  ...
  // query if we have obstacle in segment that starts at from to segment that starts at
  // to
