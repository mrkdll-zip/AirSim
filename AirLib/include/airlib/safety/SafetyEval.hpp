...
#include "airlib/common/common_utils/EnumFlags.hpp"
...
class SafetyEval {
  ...
  enum class ObsAvoidanceStrategy : uint {
    ...
    OppositeMove  // find closest obstacle free destination along opposite of desired
                  // destination
  };
  ...
  struct EvalResult {
    ...
    // obstacle info around cur and towards dest, if no obs found then cur might not be
    // evaluated
    ...
    // risk distances indicates how far we are in to risk zone, lower (<= 0) better than
    // higher cur is for risk distance around current position dest if risk distance
    // towards destination
    ...
    EvalResult()
        : is_safe(true),
          reason(SafetyViolationType_::NoSafetyViolation),
          suggested_vec(Vector3r::Zero()),
          cur_risk_dist(Utils::nan<float>()),
          dest_risk_dist(Utils::nan<float>())
    {
    }
    ...
    string toString() const
    {
      return Utils::stringf(
          "SafetyEval: is_safe=%i, reason=%X, cur_risk_dist=%f, dest_risk_dist=%f, "
          "suggested_vec=%s,"
          " cur=%s, dest=%s, cur_dest_body=%s, cur_obs=[%s], dest_obs=[%s], "
          "suggested_obs=[%s], message=%s",
          is_safe, uint(reason), cur_risk_dist, dest_risk_dist,
          VectorMath::toString(suggested_vec).c_str(),
          VectorMath::toString(cur_pos).c_str(), VectorMath::toString(dest_pos).c_str(),
          VectorMath::toString(cur_dest_body).c_str(), cur_obs.toString().c_str(),
          dest_obs.toString().c_str(), suggested_obs.toString().c_str(),
          message.c_str());
    }
  };
  ...
  void checkFence(const Vector3r& cur_pos, const Vector3r& dest_pos,
                  EvalResult& appendToResult);
  void isSafeDestination(const Vector3r& dest, const Vector3r& cur_pos,
                         const Quaternionr& quaternion, SafetyEval::EvalResult& result);
  ...
  void setSuggestedVelocity(SafetyEval::EvalResult& result,
                            const Quaternionr& quaternion);
  float adjustClearanceForPrStl(float base_clearance, float obs_confidence);

  public:
  SafetyEval(MultirotorApiParams vehicle_params, shared_ptr<IGeoFence> fence_ptr,
             shared_ptr<ObstacleMap> obs_xy);
  EvalResult isSafeVelocity(const Vector3r& cur_pos, const Vector3r& velocity,
                            const Quaternionr& quaternion);
  EvalResult isSafeVelocityZ(const Vector3r& cur_pos, float vx, float vy, float z,
                             const Quaternionr& quaternion);
  EvalResult isSafeDestination(const Vector3r& dest, const Vector3r& cur_pos,
                               const Quaternionr& quaternion);
  ...
  void setSafety(SafetyViolationType enable_reasons, float obs_clearance,
                 SafetyEval::ObsAvoidanceStrategy obs_strategy, const Vector3r& origin,
