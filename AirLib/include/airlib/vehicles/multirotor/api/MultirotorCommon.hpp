...
struct RotorParameters {
  ...
  RotorParameters(const real_T& thrust_val, const real_T& torque_scaler_val,
                  const real_T& speed_val)
      : thrust(thrust_val), torque_scaler(torque_scaler_val), speed(speed_val)
  {
  }

  void update(const real_T& thrust_val, const real_T& torque_scaler_val,
              const real_T& speed_val)
  {
    thrust        = thrust_val;
...
// Yaw mode specifies if yaw should be set as angle or angular velocity around the
// center of drone
struct YawMode {
  bool is_rate      = true;
...
struct MultirotorApiParams {
  ...
  // Below is just proportionality constant to convert from velocity to breaking
  // distance
  float vel_to_breaking_dist = 0.5f;  // ideally this should be 2X for very high speed
                                      // but for testing we are keeping it 0.5
...
struct MultirotorState {
  ...
  std::string ready_message;  // can show error message if drone is not reachable over
                              // the network or is not responding
  bool can_arm;               // indicates drone is ready to be armed

  MultirotorState()
  {
  }
  MultirotorState(const CollisionInfo& collision_val,
                  ...
                  bool ready_val, const std::string& message, bool can_arm_val)
