...
class MultirotorApiBase : public VehicleApiBase {
  ...
  virtual void commandRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate,
                                               float throttle)                     = 0;
  ...
  virtual void commandRollPitchYawrateZ(float roll, float pitch, float yaw_rate,
                                        float z)                                   = 0;
  virtual void commandAngleRatesZ(float roll_rate, float pitch_rate, float yaw_rate,
                                  float z)                                         = 0;
   ...
   * *********************************/
  virtual void setControllerGains(uint8_t controllerType, const vector<float>& kp,
                                  const vector<float>& ki, const vector<float>& kd) = 0;
  ...
  virtual float getTakeoffZ()
      const = 0;  // the height above ground for the drone after successful takeoff (Z
                  // above ground is negative due to NED coordinate system).
  // noise in difference of two position coordinates. This is not GPS or position
  // accuracy which can be very low such as 1m. the difference between two position
  // cancels out transitional errors. Typically this would be 0.1m or lower.
  ...
  // below methods gets called by default implementations of move-related commands that
  // would use a long running loop. These can be used by derived classes to do some
  // init/cleanup.
  ...
  // below method exist for any firmwares that may want to use ground truth for
  // debugging purposes
  virtual void setSimulatedGroundTruth(const Kinematics::State* kinematics,
  ...
  // return value of these function is true if command was completed without
  ...
  virtual bool moveToGPS(float latitude, float longitude, float altitude,
                         float velocity, float timeout_sec, DrivetrainType drivetrain,
                         const YawMode& yaw_mode, float lookahead,
                         float adaptive_lookahead);
  virtual bool moveByVelocityBodyFrame(float vx, float vy, float vz, float duration,
                                       DrivetrainType drivetrain,
                                       const YawMode& yaw_mode);
  virtual bool moveByVelocityZBodyFrame(float vx, float vy, float z, float duration,
                                        DrivetrainType drivetrain,
                                        const YawMode& yaw_mode);
  ...
  virtual bool moveByRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate,
                                              float throttle, float duration);
  virtual bool moveByRollPitchYawrateZ(float roll, float pitch, float yaw_rate, float z,
                                       float duration);
  virtual bool moveByAngleRatesZ(float roll_rate, float pitch_rate, float yaw_rate,
  ...
  virtual bool moveByVelocityZ(float vx, float vy, float z, float duration,
                               DrivetrainType drivetrain, const YawMode& yaw_mode);
  virtual bool moveOnPath(const vector<Vector3r>& path, float velocity,
  ...
  virtual bool moveToZ(float z, float velocity, float timeout_sec,
                       const YawMode& yaw_mode, float lookahead,
                       float adaptive_lookahead);
  virtual bool moveByManual(float vx_max, float vy_max, float z_min, float duration,
                            DrivetrainType drivetrain, const YawMode& yaw_mode);
  ...
  virtual bool hover();
  virtual RCData estimateRCTrims(float trimduration = 1, float minCountForTrim = 10,
                                 float maxTrim = 100);

  /************************* set angle gain APIs *********************************/
  virtual void setAngleLevelControllerGains(const vector<float>& kp,
                                            const vector<float>& ki,
                                            const vector<float>& kd);
  virtual void setAngleRateControllerGains(const vector<float>& kp,
                                           const vector<float>& ki,
                                           const vector<float>& kd);
  virtual void setVelocityControllerGains(const vector<float>& kp,
                                          const vector<float>& ki,
                                          const vector<float>& kd);
  virtual void setPositionControllerGains(const vector<float>& kp,
                                          const vector<float>& ki,
                                          const vector<float>& kd);

  /************************* Safety APIs *********************************/
  virtual void setSafetyEval(const shared_ptr<SafetyEval> safety_eval_ptr);
  virtual bool setSafety(SafetyEval::SafetyViolationType enable_reasons,
                         float obs_clearance,
  ...
  virtual void moveByRollPitchYawThrottleInternal(float roll, float pitch, float yaw,
                                                  float throttle);
  virtual void moveByRollPitchYawrateThrottleInternal(float roll, float pitch,
                                                      float yaw_rate, float throttle);
  virtual void moveByRollPitchYawrateZInternal(float roll, float pitch, float yaw_rate,
                                               float z);
  virtual void moveByAngleRatesZInternal(float roll_rate, float pitch_rate,
                                         float yaw_rate, float z);
  virtual void moveByAngleRatesThrottleInternal(float roll_rate, float pitch_rate,
                                                float yaw_rate, float throttle);
  ...
  /************* wait helpers ************/
  // helper function can wait for anything (as defined by the given function) up to the
  // max_wait duration (in seconds). returns true if the wait function succeeded, or
  // false if timeout occurred or the timeout is invalid.
...
public:
    ...
    UnsafeMoveException(const SafetyEval::EvalResult result_val,
...
private:
    // we need mutable here because status APIs are const and shouldn't change data
    // members
  ...
  struct PathSegment {
    ...
    PathSegment(const Vector3r& start, const Vector3r& end, float velocity,
                float path_length)
    {
      seg             = end - start;
      seg_length      = seg.norm();
...
public:
    ObsStrategyChanger(shared_ptr<SafetyEval> safety_eval_ptr,
                       SafetyEval::ObsAvoidanceStrategy new_startegy)
    {
  ...
  private:  // methods
  float setNextPathPosition(const vector<Vector3r>& path,
  ...
  void moveToPathPosition(const Vector3r& dest, float velocity,
                          DrivetrainType drivetrain,
