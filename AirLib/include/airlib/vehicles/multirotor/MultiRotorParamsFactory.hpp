...
#include "airlib/vehicles/multirotor/firmwares/arducopter/ArduCopterParams.hpp"
#include "airlib/vehicles/multirotor/firmwares/mavlink/ArduCopterSoloParams.hpp"
#include "airlib/vehicles/multirotor/firmwares/mavlink/MavLinkMultirotorApi.hpp"
#include "airlib/vehicles/multirotor/firmwares/mavlink/Px4MultiRotorParams.hpp"
#include "airlib/vehicles/multirotor/firmwares/simple_flight/SimpleFlightQuadXParams.hpp"
      ...
      std::shared_ptr<const SensorFactory> sensor_factory)
  {
    ...
    if (vehicle_setting->vehicle_type == AirSimSettings::kVehicleTypePX4) {
      config.reset(new Px4MultiRotorParams(
          *static_cast<const AirSimSettings::MavLinkVehicleSetting*>(vehicle_setting),
          sensor_factory));
    }
    else if (vehicle_setting->vehicle_type ==
             AirSimSettings::kVehicleTypeArduCopterSolo) {
