...
#include "airlib/api/VehicleSimApiBase.hpp"
...
class MultiRotorPhysicsBody : public PhysicsBody {
  ...
  virtual void update() override
  {
    ...
    // Note that controller gets updated after kinematics gets updated in
    // updateKinematics otherwise sensors will have values from previous cycle causing
    // lags which will appear as crazy jerks whenever commands like velocity is issued
  }
  ...
  // External Physics engine calls this method to keep physics bodies updated and move
  // rotors
  ...
  static void createRotors(const MultiRotorParams& params,
                           vector<RotorActuator>& rotors,
                           const Environment* environment)
  {
    rotors.clear();
    // for each rotor pose
    for (uint rotor_index = 0; rotor_index < params.getParams().rotor_poses.size();
         ++rotor_index) {
      const MultiRotorParams::RotorPose& rotor_pose =
          params.getParams().rotor_poses.at(rotor_index);
  ...
  void updateSensors(MultiRotorParams& params, const Kinematics::State& state,
                     const Environment& environment)
  {
    unused(state);
    unused(environment);
  ...
  void initSensors(MultiRotorParams& params, const Kinematics::State& state,
                   const Environment& environment)
  {
  ...
  void createDragVertices()
  {
    ...
    // Drone is seen as central body that is connected to propellers via arm. We
    // approximate central body as box of size x, y, z. The drag depends on area exposed
    // so we also add area of propellers to approximate drag they may introduce due to
    // their area. while moving along any axis, we find area that will be exposed in
    // that direction
    ...
    Vector3r drag_factor_unit =
        Vector3r(front_back_area + rotors_.size() * propeller_xsection,
    ...
    drag_faces_.clear();
    drag_faces_.emplace_back(Vector3r(0, 0, -params.body_box.z() / 2.0f),
                             Vector3r(0, 0, -1), drag_factor_unit.z());
    drag_faces_.emplace_back(Vector3r(0, 0, params.body_box.z() / 2.0f),
                             Vector3r(0, 0, 1), drag_factor_unit.z());
    drag_faces_.emplace_back(Vector3r(0, -params.body_box.y() / 2.0f, 0),
                             Vector3r(0, -1, 0), drag_factor_unit.y());
    drag_faces_.emplace_back(Vector3r(0, params.body_box.y() / 2.0f, 0),
                             Vector3r(0, 1, 0), drag_factor_unit.y());
    drag_faces_.emplace_back(Vector3r(-params.body_box.x() / 2.0f, 0, 0),
                             Vector3r(-1, 0, 0), drag_factor_unit.x());
    drag_faces_.emplace_back(Vector3r(params.body_box.x() / 2.0f, 0, 0),
                             Vector3r(1, 0, 0), drag_factor_unit.x());
  }
