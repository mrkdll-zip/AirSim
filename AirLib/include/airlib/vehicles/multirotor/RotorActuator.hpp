...
#include "airlib/common/FirstOrderFilter.hpp"
#include "airlib/physics/Environment.hpp"
#include "airlib/physics/PhysicsBodyVertex.hpp"

namespace msr {
namespace airlib {

// Rotor gets control signal as input (PWM or voltage represented from 0 to 1) which
// causes change in rotation speed and turning direction and ultimately produces force
// and thrust as output
class RotorActuator : public PhysicsBodyVertex {
  ...
  RotorActuator(const Vector3r& position, const Vector3r& normal,
                RotorTurningDirection turning_direction, const RotorParams& params,
  ...
  virtual void setWrench(Wrench& wrench) override
  {
    ...
    wrench.torque = normal * output_.torque_scaler *
                    air_density_ratio_;  // TODO: try using filtered control here
  }
                        ...
                        RotorTurningDirection turning_direction)
  {
    ...
    // see relationship of rotation speed with thrust:
    // http://physics.stackexchange.com/a/32013/14061
  ...
  {
    // update air density ration - this will affect generated force and torques by
    // rotors
