...
#include "airlib/sensors/distance/DistanceSimple.hpp"
#include "airlib/sensors/gps/GpsBase.hpp"
#include "airlib/sensors/imu/ImuBase.hpp"
#include "airlib/sensors/lidar/LidarSimple.hpp"
...
class ArduCopterApi : public MultirotorApiBase {
  ...
  ArduCopterApi(const MultiRotorParams* vehicle_params,
                const AirSimSettings::MavLinkConnectionInfo& connection_info)
  ...
  virtual Quaternionr getOrientation() const override
  {
    ...
    return Quaternionr(Utils::nan<float>(), Utils::nan<float>(), Utils::nan<float>(),
                       Utils::nan<float>());
  }
  ...
  virtual float getTakeoffZ() const override
  {
    ...
    // system. return params_.takeoff.takeoff_z;
    return 3.0;
  }
  ...
  virtual void commandMotorPWMs(float front_right_pwm, float front_left_pwm,
                                float rear_right_pwm, float rear_left_pwm) override
  {
                                               ...
                                               float throttle) override
  {
    ...
    unused(yaw_rate);
    unused(throttle);
    Utils::log("Not Implemented: commandRollPitchYawrateThrottle",
               Utils::kLogLevelInfo);
  }
  ...
  void sendSensors()
  {
    ...
    buf << "\"imu\": {" << std::fixed << std::setprecision(7)
        << "\"angular_velocity\": [" << imu_output.angular_velocity[0] << ","
        << imu_output.angular_velocity[1] << "," << imu_output.angular_velocity[2]
        << "]"
        << ","
    ...
    if (count_gps_sensors != 0) {
      ...
      buf << ","
          ...
          << "\"velocity\": {"
          << "\"world_linear_velocity\": [" << gps_output.gnss.velocity[0] << ","
          << gps_output.gnss.velocity[1] << "," << gps_output.gnss.velocity[2]
          << "]"
             "}";
    }
    ...
    const uint count_distance_sensors =
        sensors_->size(SensorBase::SensorType::Distance);
    ...
    if (count_lidars != 0) {
      ...
      for (uint i = 0; i < count_lidars; ++i) {
        const auto* lidar = static_cast<const LidarSimple*>(
            sensors_->getByType(SensorBase::SensorType::Lidar, i));
  ...
  void recvRotorControl()
  {
    ...
    while (recv_ret != sizeof(pkt)) {
      if (recv_ret <= 0) {
        Utils::log(
            Utils::stringf("Error while receiving rotor control data - ErrorNo: %d",
                           recv_ret),
            Utils::kLogLevelInfo);
      }
      else {
        Utils::log(Utils::stringf("Received %d bytes instead of %zu bytes", recv_ret,
                                  sizeof(pkt)),
                   Utils::kLogLevelInfo);
      }
