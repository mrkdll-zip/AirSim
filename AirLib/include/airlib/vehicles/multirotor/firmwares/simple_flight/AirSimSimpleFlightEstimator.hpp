...
class AirSimSimpleFlightEstimator : public simple_flight::IStateEstimator {
  ...
  // for now we don't do any state estimation and use ground truth (i.e. assume perfect
  // sensors)
  ...
  virtual simple_flight::Axis3r getAngles() const override
  {
    ...
    VectorMath::toEulerianAngle(kinematics_->pose.orientation, angles.pitch(),
  ...
  virtual simple_flight::Axis3r transformToBodyFrame(
      const simple_flight::Axis3r& world_frame_val) const override
  {
    ...
    const Vector3r& trans =
        VectorMath::transformToBodyFrame(vec, kinematics_->pose.orientation);
  ...
  {
    return AirSimSimpleFlightCommon::toSimpleFlightGeoPoint(
  ...
  virtual simple_flight::KinematicsState getKinematicsEstimated() const override
  {
    ...
    state.linear_acceleration =
        AirSimSimpleFlightCommon::toAxis3r(kinematics_->accelerations.linear);
    state.angular_acceleration =
        AirSimSimpleFlightCommon::toAxis3r(kinematics_->accelerations.angular);
