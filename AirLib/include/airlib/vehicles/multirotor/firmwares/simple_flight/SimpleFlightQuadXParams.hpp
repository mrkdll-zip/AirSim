...
#include "airlib/vehicles/multirotor/firmwares/simple_flight/SimpleFlightApi.hpp"
...
class SimpleFlightQuadXParams : public MultiRotorParams {
  public:
  SimpleFlightQuadXParams(const AirSimSettings::VehicleSetting* vehicle_setting,
  ...
  {
    return std::unique_ptr<MultirotorApiBase>(
        new SimpleFlightApi(this, vehicle_setting_));
  }
  ...
  virtual void setupParams() override
  {
    ...
    // Use connection_info_.model for the model name, see Px4MultiRotorParams for
    // example
  ...
  const AirSimSettings::VehicleSetting*
      vehicle_setting_;  // store as pointer because of derived classes
