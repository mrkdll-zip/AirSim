...
class RpcLibAdaptorsBase {
  ...
  {
    d.clear();
    for (size_t i = 0; i < s.size(); ++i) d.push_back(s.at(i).to());
  }
  ...
  {
    d.clear();
    for (size_t i = 0; i < s.size(); ++i) d.push_back(TDest(s.at(i)));
  }
  ...
  struct CollisionInfo {
    ...
    MSGPACK_DEFINE_MAP(has_collided, penetration_depth, time_stamp, normal,
    ...
    {
      return msr::airlib::CollisionInfo(has_collided, normal.to(), impact_point.to(),
                                        position.to(), penetration_depth, time_stamp,
  ...
  struct RCData {
    ...
    MSGPACK_DEFINE_MAP(timestamp, pitch, roll, throttle, yaw, left_z, right_z, switches,
  ...
  struct ProjectionMatrix {
    ...
    {
      for (auto i = 0; i < 4; ++i)
        for (auto j = 0; j < 4; ++j) matrix[i][j] = s.matrix[i][j];
    }
    ...
    msr::airlib::ProjectionMatrix to() const
    {
      ...
      for (auto i = 0; i < 4; ++i)
        for (auto j = 0; j < 4; ++j) s.matrix[i][j] = matrix[i][j];
      return s;
    }
  };
  ...
  struct DetectionInfo {
        ...
        const std::vector<msr::airlib::DetectionInfo>& request)
    {
      ...
      for (const auto& item : request) request_adaptor.push_back(DetectionInfo(item));
        ...
        const std::vector<DetectionInfo>& request_adapter)
    {
      ...
      for (const auto& item : request_adapter) request.push_back(item.to());
  ...
  struct KinematicsState {
    ...
    MSGPACK_DEFINE_MAP(position, orientation, linear_velocity, angular_velocity,
  ...
  struct ImageRequest {
        ...
        const std::vector<msr::airlib::ImageCaptureBase::ImageRequest>& request)
    {
      ...
      for (const auto& item : request) request_adaptor.push_back(ImageRequest(item));
  ...
  struct ImageResponse {
        ...
        const std::vector<ImageResponse>& response_adapter)
    {
      ...
      for (const auto& item : response_adapter) response.push_back(item.to());
        ...
        const std::vector<msr::airlib::ImageCaptureBase::ImageResponse>& response)
    {
      ...
      for (const auto& item : response) response_adapter.push_back(ImageResponse(item));
  ...
  struct MagnetometerData {
    ...
    std::vector<float>
        magnetic_field_covariance;  // not implemented in MagnetometerBase.hpp
  ...
  struct MeshPositionVertexBuffersResponse {
    ...
    MeshPositionVertexBuffersResponse(
        const msr::airlib::MeshPositionVertexBuffersResponse& s)
    {
