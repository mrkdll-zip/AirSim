...
class MagnetometerSimple : public MagnetometerBase {
  ...
  MagnetometerSimple(const AirSimSettings::MagnetometerSetting& setting =
                         AirSimSettings::MagnetometerSetting())
    ...
    switch (params_.ref_source) {
      ...
      case MagnetometerSimpleParams::ReferenceSource::ReferenceSource_DipoleModel:
        magnetic_field_true_ =
            EarthUtils::getMagField(ground_truth.environment->getState().geo_point) *
            1E4f;  // Tesla to Gauss
        break;
      default:
  ...
  Output getOutputInternal()
  {
    ...
    output.magnetic_field_body =
        VectorMath::transformToBodyFrame(
            magnetic_field_true_, ground_truth.kinematics->pose.orientation, true) *
