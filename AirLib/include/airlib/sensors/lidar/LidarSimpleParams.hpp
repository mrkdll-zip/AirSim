...
struct LidarSimpleParams {
  ...
  Pose relative_pose{
      Vector3r(0, 0, -1),  // position - a little above vehicle (especially for cars) or
                           // Vector3r::Zero()
  ...
  void initializeFromSettings(const AirSimSettings::LidarSetting& settings)
  {
    ...
    horizontal_rotation_frequency =
        settings_json.getInt("RotationsPerSecond", horizontal_rotation_frequency);
    ...
    external_controller =
        settings_json.getBool("ExternalController", external_controller);

    vertical_FOV_upper =
        settings_json.getFloat("VerticalFOVUpper", Utils::nan<float>());
    ...
    vertical_FOV_lower =
        settings_json.getFloat("VerticalFOVLower", Utils::nan<float>());
    ...
    horizontal_FOV_start =
        settings_json.getFloat("HorizontalFOVStart", horizontal_FOV_start);
    horizontal_FOV_end = settings_json.getFloat("HorizontalFOVEnd", horizontal_FOV_end);

    relative_pose.position =
        AirSimSettings::createVectorSetting(settings_json, VectorMath::nanVector());
    auto rotation = AirSimSettings::createRotationSetting(
        settings_json, AirSimSettings::Rotation::nanRotation());
