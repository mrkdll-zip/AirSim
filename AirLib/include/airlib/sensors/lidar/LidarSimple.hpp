...
#include "airlib/common/DelayLine.hpp"
#include "airlib/common/FrequencyLimiter.hpp"
...
class LidarSimple : public LidarBase {
  public:
  LidarSimple(
      const AirSimSettings::LidarSetting& setting = AirSimSettings::LidarSetting())
  ...
  void updateOutput()
  {
    ...
    // calculate the pose before obtaining the point-cloud. Before/after is a bit
    // arbitrary decision here. If the pose can change while obtaining the point-cloud
    // (could happen for drones) then the pose won't be very accurate either way.
    //
    // TODO: Seems like pose is in vehicle inertial-frame (NOT in Global NED frame).
    //    That could be a bit unintuitive but seems consistent with the
    //    position/orientation returned as part of ImageResponse for cameras and pose
    //    returned by getCameraInfo API. Do we need to convert pose to Global NED frame
