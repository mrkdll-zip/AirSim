...
#include "airlib/common/EarthUtils.hpp"
...
For Allan Variance/Deviation plots see
http://www.invensense.com/wp-content/uploads/2015/02/MPU-3300-Datasheet.pdf
*/
  struct Gyroscope {
    // angular random walk (ARW)
    real_T arw = 0.30f / sqrt(3600.0f) * M_PIf /
                 180;  // deg/sqrt(hour) converted to rad/sqrt(sec)
  ...
  struct Accelerometer {
    ...
    real_T tau = 800;
    real_T bias_stability =
        36.0f * 1E-6f * EarthUtils::Gravity;   // ug converted to m/s^2
    Vector3r turn_on_bias = Vector3r::Zero();  // assume calibration is done
  } accel;
  ...
  void initializeFromSettings(const AirSimSettings::ImuSetting& settings)
  {
    ...
    if (!std::isnan(arw)) {
      gyro.arw = arw / sqrt(3600.0f) * M_PIf /
                 180;  // //deg/sqrt(hour) converted to rad/sqrt(sec)
    }
    ...
    if (!std::isnan(bias_stability)) {
      gyro.bias_stability =
          bias_stability / 3600 * M_PIf / 180;  // deg/hr converted to rad/sec
    }
    ...
    if (!std::isnan(bias_stability)) {
      accel.bias_stability =
          bias_stability * 1E-6f * EarthUtils::Gravity;  // ug converted to m/s^2
    }
  }
};
}  // namespace airlib
