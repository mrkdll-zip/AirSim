...
class ImuSimple : public ImuBase {
  ...
  void updateOutput()
  {
    ...
    output.linear_acceleration = ground_truth.kinematics->accelerations.linear -
    ...
    output.linear_acceleration = VectorMath::transformToBodyFrame(
        output.linear_acceleration, ground_truth.kinematics->pose.orientation, true);
  ...
  void addNoise(Vector3r& linear_acceleration, Vector3r& angular_velocity)
  {
    ...
    real_T sqrt_dt =
        static_cast<real_T>(sqrt(std::max<TTimeDelta>(dt, params_.min_sample_time)));
    ...
    linear_acceleration +=
        gauss_dist.next() * accel_sigma_vrw + state_.accelerometer_bias;
    // update bias random walk
