...
#include "airlib/common/GaussianMarkov.hpp"
...
class BarometerSimple : public BarometerBase {
  ...
  BarometerSimple(const AirSimSettings::BarometerSetting& setting =
                      AirSimSettings::BarometerSetting())
      : BarometerBase(setting.sensor_name)
  {
    ...
    pressure_factor_.initialize(params_.pressure_factor_tau,
                                params_.pressure_factor_sigma, 0);

    uncorrelated_noise_ =
        RandomGeneratorGausianR(0.0f, params_.uncorrelated_noise_sigma);
    // correlated_noise_.initialize(params_.correlated_noise_tau,
    // params_.correlated_noise_sigma, 0.0f);
  ...
  Output getOutputInternal()
  {
    ...
    output.altitude = (1 - pow(pressure / EarthUtils::SeaLevelPressure, 0.190284f)) *
                      145366.45f * 0.3048f;
