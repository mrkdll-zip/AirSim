...
class GpsSimple : public GpsBase {
      ...
      : GpsBase(setting.sensor_name)
  {
    ...
    eph_filter.initialize(
        ...
        params_.eph_initial);  // starting dilution set to 100 which we will reduce over
                               // time to targeted 0.3f, with 45% accuracy within 100
                               // updates, each update occurring at 0.2s interval
    epv_filter.initialize(params_.epv_time_constant, params_.epv_final,
                          params_.epv_initial);
  }
