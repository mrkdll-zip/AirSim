...
#include "airlib/vehicles/multirotor/api/MultirotorCommon.hpp"
...
SafetyEval::SafetyEval(MultirotorApiParams vehicle_params,
                       shared_ptr<IGeoFence> fence_ptr,
                       shared_ptr<ObstacleMap> obs_xy_ptr)
    : vehicle_params_(vehicle_params), fence_ptr_(fence_ptr), obs_xy_ptr_(obs_xy_ptr)
{
  Utils::log(Utils::stringf("enable_reasons: %X, obs_strategy=%X",
                            uint(enable_reasons_), uint(obs_strategy_)));
}

void SafetyEval::checkFence(const Vector3r& dest_pos, const Vector3r& cur_pos,
                            SafetyEval::EvalResult& appendToResult)
{
  ...
  if (!allow) {
    ...
    appendToResult.message.append(common_utils::Utils::stringf(
        "Destination %s is outside geo fence and is worse off than current %s in "
        "geofence=[%s]",
...
SafetyEval::EvalResult SafetyEval::isSafeDestination(const Vector3r& dest_pos,
                                                     const Vector3r& cur_pos,
...
SafetyEval::EvalResult SafetyEval::isSafePosition(const Vector3r& cur_pos,
                                                  const Quaternionr& quaternion)
{
  SafetyEval::EvalResult result;
...
void SafetyEval::isCurrentSafer(SafetyEval::EvalResult& result)
{
  ...
  result.cur_risk_dist = adjustClearanceForPrStl(vehicle_params_.obs_clearance,
                                                 result.cur_obs.confidence) -
                         result.cur_obs.distance;
...
void SafetyEval::isSafeDestination(const Vector3r& dest_pos, const Vector3r& cur_pos,
                                   const Quaternionr& quaternion,
                                   SafetyEval::EvalResult& result)
{
  ...
  if (cur_dest_norm < vehicle_params_.distance_accuracy) {
    ...
    result.dest_obs = obs_xy_ptr_->hasObstacle(point_tick - vehicle_params_.obs_window,
                                               point_tick + vehicle_params_.obs_window);

    // less risk distance is better
    result.dest_risk_dist = cur_dest_norm +
                            adjustClearanceForPrStl(vehicle_params_.obs_clearance,
                                                    result.dest_obs.confidence) -
  ...
  if (!result.is_safe && result.reason & SafetyViolationType_::Obstacle) {
    ...
    Utils::log(Utils::stringf(
        "isSafeDestination: cur_dest_norm=%f, result=%s, quaternion=%s", cur_dest_norm,
...
{
  // 3.2 comes from inverse CDF for epsilon = 0.05 (i.e. 95% confidence), author:
  // akapoor
...
void SafetyEval::setSuggestedVelocity(SafetyEval::EvalResult& result,
                                      const Quaternionr& quaternion)
{
  ...
  for (int i = 0; i <= ticks / 2; ++i) {
    ...
    ObstacleMap::ObstacleInfo right_obs =
        obs_xy_ptr_->hasObstacle(ref_tick + i, ref_tick + i);
    ObstacleMap::ObstacleInfo left_obs =
        obs_xy_ptr_->hasObstacle(ref_tick - i, ref_tick - i);

    // find right and left risk distances
    float right_risk_dist =
        adjustClearanceForPrStl(vehicle_params_.obs_clearance, right_obs.confidence) -
        right_obs.distance;
    float left_risk_dist =
        adjustClearanceForPrStl(vehicle_params_.obs_clearance, left_obs.confidence) -
        left_obs.distance;
    ...
    if (right_risk_dist <= 0 || left_risk_dist <= 0) {
      int suggested_tick =
          right_risk_dist < left_risk_dist ? right_obs.tick : left_obs.tick;
      ...
      const Vector3r suggested_body =
          Vector3r(std::cos(suggested_angle), std::sin(suggested_angle), 0)
              .normalized();
      result.suggested_vec =
          VectorMath::transformToWorldFrame(suggested_body, quaternion, true);

      Utils::log(
          Utils::stringf("right_risk_dist=%f, left_risk_dist=%f, suggested_tick=%i, "
                         "suggested_angle=%f",
                         right_risk_dist, left_risk_dist, suggested_tick,
                         suggested_angle, suggested_angle));

      break;  // if none found then suggested_vec is left as zero vec, meaning enter
              // hover mode
    }
  }
}

SafetyEval::EvalResult SafetyEval::isSafeVelocityZ(const Vector3r& cur_pos, float vx,
                                                   float vy, float z,
                                                   const Quaternionr& quaternion)
{
  SafetyEval::EvalResult result;
...
Vector3r SafetyEval::getDestination(const Vector3r& cur_pos,
                                    const Vector3r& velocity) const
{
  ...
  return velocity_mag >= vehicle_params_.distance_accuracy
             ? (cur_pos + (velocity / velocity_mag) * dest_pos_dist)
             : cur_pos;
}

SafetyEval::EvalResult SafetyEval::isSafeVelocity(const Vector3r& cur_pos,
...
void SafetyEval::setSafety(SafetyViolationType enable_reasons, float obs_clearance,
                           SafetyEval::ObsAvoidanceStrategy obs_strategy,
                           const Vector3r& origin, float xy_length, float max_z,
                           float min_z)
{
  if (!origin.hasNaN() && !std::isnan(xy_length) && !std::isnan(max_z) &&
      !std::isnan(min_z))
