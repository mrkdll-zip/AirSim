...
bool MultirotorApiBase::goHome(float timeout_sec)
{
  ...
  return moveToPosition(0, 0, 0, 0.5f, timeout_sec, DrivetrainType::MaxDegreeOfFreedom,
                        YawMode::Zero(), -1, 1);
}

bool MultirotorApiBase::moveByVelocityBodyFrame(float vx, float vy, float vz,
                                                float duration,
                                                ...
                                                const YawMode& yaw_mode)
{
  ...
  VectorMath::toEulerianAngle(getKinematicsEstimated().pose.orientation, pitch, roll,
                              yaw);
...
bool MultirotorApiBase::moveByVelocityZBodyFrame(float vx, float vy, float z,
                                                 float duration,
                                                 DrivetrainType drivetrain,
                                                 const YawMode& yaw_mode)
{
...
bool MultirotorApiBase::moveByMotorPWMs(float front_right_pwm, float rear_left_pwm,
...
bool MultirotorApiBase::moveByRollPitchYawZ(float roll, float pitch, float yaw, float z,
                                            float duration)
{
  SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveByRollPitchYawThrottle(float roll, float pitch, float yaw,
...
bool MultirotorApiBase::moveByRollPitchYawrateThrottle(float roll, float pitch,
...
bool MultirotorApiBase::moveByRollPitchYawrateZ(float roll, float pitch, float yaw_rate,
...
bool MultirotorApiBase::moveByAngleRatesThrottle(float roll_rate, float pitch_rate,
                                                 float yaw_rate, float throttle,
                                                 float duration)
{
  SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveByVelocity(float vx, float vy, float vz, float duration,
                                       DrivetrainType drivetrain,
                                       const YawMode& yaw_mode)
{
  SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveByVelocityZ(float vx, float vy, float z, float duration,
                                        DrivetrainType drivetrain,
                                        const YawMode& yaw_mode)
{
  SingleTaskCall lock(this);
...
bool MultirotorApiBase::moveOnPath(const vector<Vector3r>& path, float velocity,
                                   ...
                                   float adaptive_lookahead)
{
  ...
  if (lookahead == 0)
    ...
    if (command_period_dist > lookahead)
      throw std::invalid_argument(Utils::stringf(
          "lookahead value %f is too small for velocity %f. It must be at least %f",
          lookahead, velocity, command_period_dist));
    if (getDistanceAccuracy() > lookahead)
      throw std::invalid_argument(Utils::stringf(
          "lookahead value %f is smaller than drone's distance accuracy %f.", lookahead,
          getDistanceAccuracy()));
  }
  else {
    ...
    Utils::log(Utils::stringf("lookahead = %f, adaptive_lookahead = %f", lookahead,
                              adaptive_lookahead));
  }
  ...
  // add last segment as zero length segment so we have equal number of segments and
  // points. path_segs[i] refers to segment that starts at point i
  ...
  setNextPathPosition(path3d, path_segs, cur_path_loc, lookahead + lookahead_error,
                      next_path_loc);
  float overshoot = 0;
  float goal_dist = 0;
  ...
  while (!waiter.isTimeout() &&
    ...
    float path_length_remaining = path_length -
                                  path_segs.at(cur_path_loc.seg_index).seg_path_length -
                                  cur_path_loc.offset;
    if (seg_velocity > getMultirotorApiParams().min_vel_for_breaking &&
        path_length_remaining <= breaking_dist) {
      seg_velocity = getMultirotorApiParams().breaking_vel;
      // Utils::logMessage("path_length_remaining = %f, Switched to breaking vel %f",
      // path_length_remaining, seg_velocity);
    }
    ...
    /*  Below, P is previous position on path, N is next goal and C is our current
position.
...
Note that PC could be at any angle relative to PN, including 0 or -ve. We increase
lookahead distance by the amount of |PC|. For this, we project PC on to PN to get vector
PC' and length of CC'is our adaptive lookahead error by which we will increase lookahead
distance.
...
One good test case is if C just keeps moving perpendicular to the path (instead of along
the path). In that case, we expect next goal to come up and down by the amount of
lookahead_error. However under no circumstances we should go back on the path (i.e.
current pos on path can only move forward).
*/
    ...
    if (!goal_vect.isZero()) {  // goal can only be zero if we are at the end of path
      ...
      goal_dist                       = actual_vect.dot(
                                goal_normalized);  // dist could be -ve if drone moves away from goal
        ...
        if (error > lookahead_error) {
          ...
          if (lookahead_error_increasing > 1E5) {
            throw std::runtime_error(
                "lookahead error is continually increasing so we do not have safe "
    ...
    // Utils::logMessage("PF: cur=%s, goal_dist=%f, cur_path_loc=%s, next_path_loc=%s,
    // lookahead_error=%f",
    ...
    if (goal_dist >= 0) {
      overshoot =
          setNextPathPosition(path3d, path_segs, cur_path_loc, goal_dist, cur_path_loc);
      if (overshoot)
...
bool MultirotorApiBase::moveToGPS(float latitude, float longitude, float altitude,
                                  ...
                                  float lookahead, float adaptive_lookahead)
{
  ...
  if (!std::isnan(getHomeGeoPoint().latitude) &&
      !std::isnan(getHomeGeoPoint().longitude) &&
      !std::isnan(getHomeGeoPoint().altitude)) {
    vector<Vector3r> path{
        msr::airlib::EarthUtils::GeodeticToNed(target, getHomeGeoPoint())};
    return moveOnPath(path, velocity, timeout_sec, drivetrain, yaw_mode, lookahead,
                      adaptive_lookahead);
  }
  else {
    vector<Vector3r> path{
        Vector3r(getPosition().x(), getPosition().y(), getPosition().z())};
    return moveOnPath(path, velocity, timeout_sec, drivetrain, yaw_mode, lookahead,
                      adaptive_lookahead);
  }
}

bool MultirotorApiBase::moveToPosition(float x, float y, float z, float velocity,
                                       float timeout_sec, DrivetrainType drivetrain,
                                       const YawMode& yaw_mode, float lookahead,
                                       float adaptive_lookahead)
{
                                ...
                                float adaptive_lookahead)
{
  ...
  return moveOnPath(path, velocity, timeout_sec, DrivetrainType::MaxDegreeOfFreedom,
                    yaw_mode, lookahead, adaptive_lookahead);
}

bool MultirotorApiBase::moveByManual(float vx_max, float vy_max, float z_min,
                                     ...
                                     const YawMode& yaw_mode)
{
  ...
  do {
    ...
    if (rc_data.is_valid &&
        (rc_data.timestamp == 0 ||
         age <=
             kMaxMessageAge)) {  // if rc message timestamp is not set OR is not too old
      if (rc_data_trims_.is_valid)
        rc_data.subtract(rc_data_trims_);

      // convert RC commands to velocity vector
      const Vector3r vel_word(rc_data.pitch * vy_max / kMaxRCValue,
                              rc_data.roll * vx_max / kMaxRCValue, 0);
      Vector3r vel_body =
          VectorMath::transformToBodyFrame(vel_word, starting_quaternion, true);
  ...
  // if timeout occurred then command completed successfully otherwise it was
  // interrupted
...
bool MultirotorApiBase::rotateToYaw(float yaw, float timeout_sec, float margin)
{
  ...
  return waitForFunction(
             [&]() {
               if (isYawWithinMargin(
                       yaw_target,
                       margin)) {  // yaw is within margin, then trying to stop rotation
                 moveToPositionInternal(start_pos,
                                        stop_yaw_mode);  // let yaw rate be zero
...
void MultirotorApiBase::setAngleLevelControllerGains(const vector<float>& kp,
                                                     const vector<float>& ki,
...
void MultirotorApiBase::setAngleRateControllerGains(const vector<float>& kp,
                                                    const vector<float>& ki,
                                                    const vector<float>& kd)
{
...
void MultirotorApiBase::setVelocityControllerGains(const vector<float>& kp,
                                                   const vector<float>& ki,
                                                   const vector<float>& kd)
{
...
void MultirotorApiBase::setPositionControllerGains(const vector<float>& kp,
                                                   const vector<float>& ki,
                                                   const vector<float>& kd)
{
...
bool MultirotorApiBase::hover()
{
  ...
  return moveToZ(getPosition().z(), 0.5f, Utils::max<float>(), YawMode{true, 0}, 1.0f,
                 false);
}
...
void MultirotorApiBase::moveByRC(const RCData& rc_data)
{
  ...
  throw VehicleCommandNotImplementedException(
      "moveByRC API is not implemented for this multirotor");
}
...
void MultirotorApiBase::moveToPositionInternal(const Vector3r& dest,
                                               const YawMode& yaw_mode)
{
...
void MultirotorApiBase::moveByRollPitchYawZInternal(float roll, float pitch, float yaw,
                                                    float z)
{
...
void MultirotorApiBase::moveByRollPitchYawThrottleInternal(float roll, float pitch,
...
void MultirotorApiBase::moveByRollPitchYawrateThrottleInternal(float roll, float pitch,
                                                               float yaw_rate,
                                                               float throttle)
{
  if (safetyCheckVelocity(getVelocity()))
...
void MultirotorApiBase::moveByAngleRatesZInternal(float roll_rate, float pitch_rate,
                                                  float yaw_rate, float z)
{
  if (safetyCheckVelocity(getVelocity()))
...
// executes a given function until it returns true. Each execution is spaced apart at
// command period. return value is true if exit was due to given function returning
// true, otherwise false (due to timeout)
...
RCData MultirotorApiBase::estimateRCTrims(float trimduration, float minCountForTrim,
                                          float maxTrim)
{
  ...
  if (rc_data_trims_.isAnyMoreThan(maxTrim)) {
    rc_data_trims_.is_valid = false;
    Utils::log(Utils::stringf("RC trims does not seem to be valid: %s",
                              rc_data_trims_.toString().c_str()));
  }
...
void MultirotorApiBase::moveToPathPosition(const Vector3r& dest, float velocity,
                                           DrivetrainType drivetrain,
                                           /* pass by value */ YawMode yaw_mode,
                                           float last_z)
{
  unused(last_z);
  // validate dest
  if (dest.hasNaN())
    throw std::invalid_argument(
        VectorMath::toString(dest, "dest vector cannot have NaN: "));
  ...
  if (cur_dest_norm <
    ...
    // Utils::logMessage("velocity_vect=%s",
    // VectorMath::toString(velocity_vect).c_str());
  }
    ...
    Utils::log(Utils::stringf("Too close dest: cur_dest_norm=%f, expected_dist=%f",
                              cur_dest_norm, expected_dist));
    velocity_vect = (cur_dest / cur_dest_norm) * (cur_dest_norm / getCommandPeriod());
  }

  // send commands
  // try to maintain altitude if path was in XY plan only, velocity based control is not
  // as good
  if (std::abs(cur.z() - dest.z()) <=
      getDistanceAccuracy())  // for paths in XY plan current code leaves z untouched,
...
bool MultirotorApiBase::setSafety(SafetyEval::SafetyViolationType enable_reasons,
                                  float obs_clearance,
                                  SafetyEval::ObsAvoidanceStrategy obs_startegy,
                                  float obs_avoidance_vel, const Vector3r& origin,
                                  float xy_length, float max_z, float min_z)
{
  if (safety_eval_ptr_ == nullptr)
    throw std::invalid_argument(
        "The setSafety call requires safety_eval_ptr_ to be set first");

  // default strategy is for move. In hover mode we set new strategy temporarily
  safety_eval_ptr_->setSafety(enable_reasons, obs_clearance, obs_startegy, origin,
                              xy_length, max_z, min_z);
...
bool MultirotorApiBase::emergencyManeuverIfUnsafe(const SafetyEval::EvalResult& result)
{
  if (!result.is_safe) {
    if (result.reason == SafetyEval::SafetyViolationType_::Obstacle) {
      ...
      if (safety_eval_ptr_->getObsAvoidanceStrategy() !=
          SafetyEval::ObsAvoidanceStrategy::RaiseException) {
        // get suggested velocity vector
        Vector3r avoidance_vel =
            getObsAvoidanceVelocity(result.cur_risk_dist, obs_avoidance_vel_) *
            result.suggested_vec;

        // use the unchecked command
        commandVelocityZ(avoidance_vel.x(), avoidance_vel.y(), getPosition().z(),
                         YawMode::Zero());
...
bool MultirotorApiBase::safetyCheckVelocity(const Vector3r& velocity)
{
  ...
  const auto& result =
      safety_eval_ptr_->isSafeVelocity(getPosition(), velocity, getOrientation());
...
bool MultirotorApiBase::safetyCheckVelocityZ(float vx, float vy, float z)
{
  ...
  const auto& result =
      safety_eval_ptr_->isSafeVelocityZ(getPosition(), vx, vy, z, getOrientation());
  return emergencyManeuverIfUnsafe(result);
}
bool MultirotorApiBase::safetyCheckDestination(const Vector3r& dest_pos)
{
  ...
  const auto& result =
      safety_eval_ptr_->isSafeDestination(getPosition(), dest_pos, getOrientation());
  return emergencyManeuverIfUnsafe(result);
}

float MultirotorApiBase::setNextPathPosition(const vector<Vector3r>& path,
                                             const vector<PathSegment>& path_segs,
                                             const PathPosition& cur_path_loc,
                                             float next_dist,
                                             PathPosition& next_path_loc)
{
  ...
  while (i < path.size() - 1) {
    ...
    if (seg.seg_length > 0 &&  // protect against duplicate points in path, normalized
                               // seg will have NaN
        seg.seg_length >= next_dist + offset) {
      next_path_loc.seg_index = i;
      next_path_loc.offset =
          next_dist + offset;  // how much total distance we will travel on this segment
    ...
    if (&cur_path_loc == &next_path_loc)
      Utils::log(Utils::stringf("segment %d done: x=%f, y=%f, z=%f", i, path.at(i).x(),
                                path.at(i).y(), path.at(i).z()));
...
void MultirotorApiBase::adjustYaw(const Vector3r& heading, DrivetrainType drivetrain,
                                  YawMode& yaw_mode)
{
  // adjust yaw for the direction of travel in forward-only mode
  if (drivetrain == DrivetrainType::ForwardOnly && !yaw_mode.is_rate) {
    if (heading.norm() > getDistanceAccuracy()) {
      yaw_mode.yaw_or_rate =
          yaw_mode.yaw_or_rate + (std::atan2(heading.y(), heading.x()) * 180 / M_PIf);
      yaw_mode.yaw_or_rate = VectorMath::normalizeAngle(yaw_mode.yaw_or_rate);
    }
    else
      yaw_mode.setZeroRate();  // don't change existing yaw if heading is too small
                               // because that can generate random result
  }
                                          ...
                                          float max_factor, float min_factor) const
{
  ...
  lookahead = std::max(
      lookahead, getDistanceAccuracy() * 1.5f);  // 50% more than distance accuracy
  return lookahead;
}

float MultirotorApiBase::getObsAvoidanceVelocity(float risk_dist,
