...
#include "airlib/common/common_utils/MinWinDefines.hpp"
...
// unicode characters on both Windows, OSX and Linux.
std::string FileSystem::createDirectory(const std::string& fullPath)
{
  ...
  if (success != 0 && errno != EEXIST)
    throw std::ios_base::failure(
        Utils::stringf("mkdir failed for path %s with errorno %i and message %s",
...
std::string FileSystem::getUserDocumentsFolder()
{
  ...
  if (0 ==
      SHGetFolderPath(NULL, CSIDL_MYDOCUMENTS | CSIDL_FLAG_CREATE, NULL, 0, szPath)) {
    std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t> converter;
...
std::string FileSystem::getExecutableFolder()
{
  ...
  if (NULL != hModule) {
    ...
    throw std::invalid_argument(Utils::stringf(
        "Error getting executable folder - hModule is null. Last error = %d", hr));
  }
